// idioma para la datatable
var idioma =

    {
        "decimal": ",",
        "thousands": ".",
        "sProcessing":     "Procesando...",
        "sLengthMenu":     "Mostrar _MENU_ registros",
        "sZeroRecords":    "No se encontraron resultados",
        "sEmptyTable":     "Ningún dato disponible en esta tabla",
        "sInfo":           "Mostrando registros del _START_ al _END_ de un total de _TOTAL_ registros",
        "sInfoEmpty":      "Mostrando registros del 0 al 0 de un total de 0 registros",
        "sInfoFiltered":   "(filtrado de un total de _MAX_ registros)",
        "sInfoPostFix":    "",
        "sSearch":         "Buscar:",
        "sUrl":            "",
        "sInfoThousands":  ",",
        "sLoadingRecords": "Cargando...",
        "oPaginate": {
            "sFirst":    "Primero",
            "sLast":     "Último",
            "sNext":     "Siguiente",
            "sPrevious": "Anterior"
        },
        "oAria": {
            "sSortAscending":  ": Activar para ordenar la columna de manera ascendente",
            "sSortDescending": ": Activar para ordenar la columna de manera descendente"
        },
        "buttons": {
            "copyTitle": 'Información copiada',
            "copyKeys": 'Use your keyboard or menu to select the copy command',
            "copySuccess": {
                "_": '%d filas copiadas al portapapeles',
                "1": '1 fila copiada al portapapeles'
            },

            "pageLength": {
            "_": "Mostrar %d filas",
            "-1": "Mostrar Todo"
            }
        }
    };


$(document).ready(function()
{

    // tabla que muestra los Usuarios de la APP Móvil -

    var table_movil = $('#table-movil').DataTable({

        "scrollX": true,
        "language": idioma,
        "ajax": {
            "headers": {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
            "data":{ "tipo" : 1},
            "url": "/admin/mobile/getdataMobile",
            "type": "POST",

        },

        "columns":
		[
            { data: 'id', title: 'id' },
            { data: 'mobileName' , title: 'Nombre Completo'},
            { data: 'mobileEmail' , title: 'Correo_e'},
            { data: null,title : 'Acciones',  orderable: false, defaultContent: 'action',render:

                function(data,type,row){

                    var action = "<div class='btn-group'>" ;
                    action += "<a href='#' id-delete=" + data.id + " class='delete btn btn-danger' role='button'><i class='fa fa-trash'></i></a> ";
                    action += "<a href='#' class='edit btn btn-success'><i class='fa fa-edit'></i></a>";
                    action += "</div>" ;
                    return  action ;
                },

            },

        ],
            order: [[1, 'asc']]
    });
    // añade la funcionalidad de edición del usuario

    $('#table-movil tbody').on( 'click', 'a.edit', function () {

        var data = table_movil.row( $(this).parents("tr") ).data();
        $('#edit-color').addClass('table-primary');
        table_movil.$('tr.table-primary').removeClass('table-primary');
        $('#edit-color').addClass('table-primary');
        $(this).parents("tr").addClass('table-primary');

        $('#mobileName').val(data.mobileName);
        $('#mobileEmail').val(data.mobileEmail);
        $('#password').val('');
        $('#editar').val(data.id);
        $('#botonAdd').removeClass('btn-info' );
        $('#botonAdd').addClass('btn-warning ');
        $('#botonAdd').html('Modificar usuario');

    });
    // da funcionalidad al boton cancelar

    $('#Cancel').on('click',function(){
        $('#movilName').val('');
        $('#movilEmail').val('');
        $('#password').val('');
        $('#editar').val('');
        $('#botonAdd').removeClass('btn-warning');
        $('#botonAdd').addClass('btn-success');
        $('.form-control').removeClass('is-valid');
        $('#edit-color').removeClass('table-primary');
        $('#botonAdd').html('Añadir Usuario');
        table_movil.$('tr.table-primary').removeClass('table-primary');
    })

    // validate para comprobar que el password no está vacio si estas creando un nuevo usuario
    $.validator.addMethod("cPassword", function( value, element){

        var editar      = $('#editar').val();
        var password    = $('#password').val();

        // si retorna true la validacion es buena y continua sin alerta

        if (editar ==''){ // si está en modo NUEVO USER
            if (password =='') return false;
            else return true
        }
        else return true; // si esta en modo EDICION

    });

    // validate para comprobar que no existe el email
    $.validator.addMethod("cExiste", function( value, element){

        var editar        = $('#editar').val();
        var mobileEmail   = $('#mobileEmail').val();
        if (mobileEmail=='') return false;
        //return false; // si retorna true la validacion es buena y continua sin alerta
        $.ajax({
            url : '/admin/mobile/getexisteEmail',
            headers: {
                     'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            type : 'POST',
            data: {
                editar: editar,
                mobileEmail: mobileEmail
            },
            async: false,
            dataType : 'json',
            success : function(json) {

                if (json['message'] == 'existe') resultado = false;
                if (json['message'] == 'noExiste') resultado = true;
            }
        });
        return resultado;

    });

    // plugin validate para validar el formulario de añadir Usuario de Movil
    $('#form-mobile').validate({
        rules: {
                mobileEmail:     {cExiste: true },
                mobileName:      {required: true },
                password :       {cPassword: true}
            },
        messages: {
            mobileEmail:  "Este campo es obligatorio y no puede estar repetido",
            mobileName :  "Este campo es obligatorio",
            password   :  "Este campo es obligatorio para un nuevo usuario"
        },
        errorPlacement : function(error, element) {
            $(element).closest('.form-group').find('.help-block').html(error.html());
            $(element).closest('.form-group').find('.help-block').addClass('text-danger');
        },
        highlight : function(element) {
            //$(element).closest('.form-group').removeClass('has-success').addClass('has-error');
            $(element).addClass('is-invalid');
            $(element).find('.form-control').css('display', 'initial');
        },
        unhighlight: function(element, errorClass, validClass) {
            //$(element).closest('.form-group').removeClass('has-error').addClass('has-success');
            //$(element).closest('.form-group').find('.help-block').html('');
            $(element).removeClass('is-invalid');
            $(element).addClass('is-valid');
            $(element).closest('.form-group').find('.help-block').html('');
        },
        submitHandler: function(form) {

            $.ajax({
                url : '/admin/mobile/storeMobile',
                headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data : {
                    'editar'     :$('#editar').val(),
                    'mobileName' :$('#mobileName').val(),
                    'mobileEmail':$('#mobileEmail').val(),
                    'password'   :$('#password').val(),

                },
                type : 'POST',
                dataType : 'json',
                success : function(data) {

                    Swal.fire({
                        icon: 'success',
                        title: 'Registro de usuario movil completado!',
                        text: 'Los datos se han almacenado correctamente',
                        footer: 'Movil'
                    })
                    table_movil.ajax.reload(null,false);
                    $('#movilName').val('');
                    $('#movilEmail').val('');
                    $('#password').val('');
                    $('#editar').val('');
                    $('#botonAdd').removeClass('btn-warning');
                    $('#botonAdd').addClass('btn-success');
                    $('.form-control').removeClass('is-valid');
                    $('#edit-color').removeClass('table-primary');
                    $('#botonAd').html('Añadir Usuario');

                }
            })
        }

    });

        // boton de borrar contacto movil
        $('#table-address tbody').on( 'click', 'a.delete', function (e) {
            e.preventDefault();
            var rowData = table_address.row( $(this).parents("tr") ).data();
            address_id  = rowData['id']

            Swal.fire({
                title: 'Estas seguro que quieres eliminar este contacto',
                text: "Esta acción no tiene marcha atras, se perderan todos los datos de este contacto",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                cancelButtonText: 'Cancelar',
                confirmButtonText: 'SI, quiero borrar el contacto !'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url : '/admin/mobile/deleteuserMobile',
                        headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        type : 'POST',
                        data: {
                            'mobile_id': mobile_id,
                        },
                        dataType : 'json',
                        success : function(json) {
                            table_address.ajax.reload(null,false);
                            if (json.message == true){
                                limpiaForm()
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Contacto Eliminado!',
                                    text: 'Todos los datos de '+ json.address_name + ' han sido eliminados.',
                                    footer: 'Address'
                                })
                            }else{
                                Swal.fire({
                                    icon: 'error',
                                    title: 'contacto NO Eliminado!',
                                    text: json.address_name,
                                    footer: 'Address'
                                })
                            }
                        }
                    });
                }
            })
        });

});
