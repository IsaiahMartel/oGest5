...
class PrintController extends Controller
{
    ...
    public function printprojectMultiple(Request $request){
        ...
        $configTitular    = Season::find($season_id)->configs->configTitular;
        $styleSheduleCell =
        [
        ...
        foreach ($project_ids as $key => $project_id) {
            ...
            $dateDiaFechaIni  = Carbon::parse($project->projectDateIni , 'UTC')->locale('es')->isoFormat('D MMMM');
            ...
            $templateWord->setValue('configTitular#'.$k,$configTitular);
            ...
            $completa = Season::find($season_id)->configs->configCompleta;
            $reducida = Season::find($season_id)->configs->configReducida;
            $sinCuerda = Season::find($season_id)->configs->configSin;
            $pd = Season::find($season_id)->configs->configPd;
            $obras = collect($playlists)->map(function ($playlist) use($k,$completa,$reducida,$sinCuerda,$pd) {
                ...
                $playlist->playlistString = str_replace(',', '/', $playlist->playlistString);
                ...
                if (count($stringDescomp) != 5 ){
                    switch ($playlist->playlistString) {
                        case 'completa' :
                        case $completa:
                            $stringDescomp = explode('/',$completa);
                            $cuerda           = 'completa';
                            break;
                        case 'reducida' :
                        case $reducida :
                            $stringDescomp = explode('/',$reducida);
                            $cuerda           = 'reducida';
                            break;
                        case 'por determinar' :
                        case $pd:
                            $stringDescomp = explode('/',$pd);
                            $cuerda           = 'por determinar';
                            break;
                        case '' :
                        case 'sin cuerda':
                        case $sinCuerda :
                            $stringDescomp = explode('/',$sinCuerda);
                            $cuerda           = 'sin cuerda';
                            break;
                ...
                $nombreObra = str_replace('<','(',$playlist->workName);
                $nombreObra = str_replace('>',')',$nombreObra);
                $nombreObra = str_replace('&','AND',$playlist->workName);
                return
                [
                    'order#'.$k         => $playlist->playlistOrder,
                    'composer#'.$k      => $compositor,
                    'workDuration#'.$k  => ($playlist->workName == 'PAUSA')? '' : substr($playlist->workDuration,0,5),
                    'playlistString#'.$k=> ($playlist->workName == 'PAUSA')? '' : $playlist->playlistString,
                    'pausa#'.$k         => ($playlist->workName == 'PAUSA')? 'PAUSA' : '',
                    'workName#'.$k      => ($playlist->workName == 'PAUSA')? '' :  $nombreObra,
                    ...
                    'trombone#'.$k      => ($playlist->workName == 'PAUSA' )? '' :'Tbn: '.$playlist->trombone,
                    ...
                    'tuba#'.$k          => ($playlist->workName == 'PAUSA' )? '' :'Tuba: '.$playlist->tuba,
