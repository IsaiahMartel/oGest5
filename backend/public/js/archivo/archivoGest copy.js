...
//detecta cambio de evento
            ...
            $.ajax({
                ...
                success : function(data)
                {
                    if (data.projectArchivo == 'Terminado'){
                        ...
                        cargaInstrumentPlaylistTerminado($('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val())
                        $('#proceso').hide( "drop", { direction: "left" }, "slow" );
                        setTimeout(function() {
                            $('#status').show( "drop", { direction: "left" }, "slow" );
                        },800);
                    }else{
                        cargaPlaylist( $('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val(),true);
                        cargaIntegrantesTerminado($('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val());
                        rellenaSheduleTerminado($('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val());
                        //rellenaShedule($('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val(),false);
    ...
    var table_works = $('#table-works').DataTable(
    {
        ...
        "columns": //far fa-arrow-alt-circle-right
        [
...
// Acción de añadir elemento (Obra a la Lista)
    ...
    $('#table-works tbody').on( 'click', 'a.add', function (e) {
        ...
        $.ajax({
            ...
            success : function(data)
            {
                if (data.message == true) {
                    ...
                    var workDuration      = obra.workDuration;
                    var work_id           = data.id;
        ...
        //var idsInOrder = $('#listComp').sortable('toArray', { attribute: 'data-work' });

        //console.log(idsInOrder);

    })

// botón para añadir PAUSA a la lista
...
// boton guardar del modal playlist
    ...
    $('#guardarPlaylist').on('click',function (e) {
        e.preventDefault();
        //compruebo si hay datos en WOKS, si no hay datos pregunto para guardar en WORKS y PLAYLIST
        $.ajax({
            ...
            success : function(data)
            {
                ...
                {
                    Swal.fire({
                        title: 'He detectado que la obra no tiene instrumentacion en la Base de datos de Work',
                        text: "¿ Queres clonar estos datos en la base de datos de la Obra ?",
                        icon: 'warning',
                        showCancelButton: true,
                        showDenyButton: true,
                        confirmButtonColor: '#3085d6',
...
// Funcion para montar el li de los aistentes
    function montaInterprete(grupo,apellido,nombre,address_id) {

    var inter ='<li id="'+address_id +'" data-group="'+ grupo + '" >';
        inter +='   <div class="row">';
    ...
    /* detecta el cambio de ESTADO para bloquear interaccion */

    $("#selectStatusTerminado").change(function(){
        Swal.fire({
            title: 'Quieres cambiar de estado?',
            text: "Esto enviará una alerta de cambio!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Si, cambia estado!'
          }).then((result) => {
            if (result.isConfirmed) {
                rellenaSheduleTerminado($('#selectSeasonName').val(),$('#selectEventName').val(),$('#projectLevel').val());
                $('#status').hide( "drop", { direction: "left" }, "slow" );
                setTimeout(function() {
